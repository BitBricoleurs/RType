...
size_t EntityFactory::createBaseMob(
    ...
    Utils::Vect2 velocity, int maxHealth, int damageValue, int player, float scale,
    float rotation, Utils::ColorR tint, int layer, bool droppowerup) {
...
size_t EntityFactory::createBossMob(
    ...
    const std::string &deathSpriteSheetPath, int deathSpriteSheetHeight,
    int deathSpriteSheetWidth, int deathFrames, Utils::Vect2 pos,
    Utils::Vect2 velocity, int maxHealth, int damageValue, int playerA, int stageValue,
    float scale, float rotation, Utils::ColorR tint, int layer, bool droppowerup) {
...
size_t EntityFactory::createPlayer(GameEngine::ComponentsContainer &container,
                                   ...
                                   float rotation, Utils::ColorR tint, int typeBullet, int layer) {
  size_t entityId = createBaseEntity(
      container, spriteSheetPath, spriteSheetHeight, spriteSheetWidth, frames,
      twoDirections, reverse, pos, velocity, playerA, scale, rotation, tint, layer);

  auto healthComponent = std::make_shared<Health>(maxHealth);
  auto shooterComp = std::make_shared<Shooter>(Utils::Vect2(bulletStartX, bulletStartY), typeBullet);
...
size_t EntityFactory::createBaseEntity(
  ...
  auto AABBComponent = std::make_shared<PhysicsEngine::AABBComponent2D>(
      pos,
      Utils::Vect2(pos.x + spriteAnimationComponent->frameWidth * scale,
                        pos.y + spriteAnimationComponent->frameHeight * scale));
  auto rectangleCollider =
      std::make_shared<PhysicsEngine::RectangleColliderComponent2D>(
          Utils::rect(0, 0, spriteAnimationComponent->frameWidth * scale,
