...
void ForcePodSpawn::update(GameEngine::ComponentsContainer &componentsContainer, GameEngine::EventHandler &eventHandler)
{
    ...
    if (anyEvent.first == "ForcePodSpawn") {
        ...
        Utils::ColorR tint = {255, 255, 255, 225};

        componentsContainer.bindComponentToEntity(
            entityId, std::make_shared<RenderEngine::SpriteComponent>(
                          "assets/force_pod_n1.gif", Utils::Vect2(0, posY),
                          Utils::rect(0, 0, 22, 16), 10, 2.5, 0, tint));
        componentsContainer.bindComponentToEntity(entityId, std::make_shared<PhysicsEngine::AABBComponent2D>(Utils::Vect2(0, posY), Utils::Vect2(22, 16)));
        componentsContainer.bindComponentToEntity(entityId, std::make_shared<PhysicsEngine::RectangleColliderComponent2D>(Utils::rect(0, 0, 22, 16)));
        componentsContainer.bindComponentToEntity(entityId, std::make_shared<PhysicsEngine::VelocityComponent>(Utils::Vect2(3, 0)));
        componentsContainer.bindComponentToEntity(entityId, std::make_shared<PhysicsEngine::MovementComponent>());
        componentsContainer.bindComponentToEntity(entityId, std::make_shared<PhysicsEngine::PositionComponent2D>(Utils::Vect2(0, posY)));
        componentsContainer.bindComponentToEntity(entityId, std::make_shared<IsForcePod>());
        componentsContainer.bindComponentToEntity(entityId, std::make_shared<Shooter>(Utils::Vect2(55, -13), 0));
